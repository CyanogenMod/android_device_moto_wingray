...
#define LOG_TAG "CM Stingray Power HAL"
#include <utils/Log.h>
...
static void stingray_power_hint(struct power_module *module, power_hint_t hint, void *data) {
    ...
    case POWER_HINT_CPU_BOOST:
        if (boostpulse_open(stingray) >= 0) {
            ...
            len = write(stingray->boostpulse_fd, buf, strlen(buf));
            ...
            if (len < 0) {
                ...
                stingray->boostpulse_fd = -1;
                stingray->boostpulse_warned = 0;
                pthread_mutex_unlock(&stingray->lock);
            }
        }
        break;
...
struct stingray_power_module HAL_MODULE_INFO_SYM = {
    base: {
        common: {
